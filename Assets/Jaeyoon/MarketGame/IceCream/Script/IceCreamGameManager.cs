...
public class IceCreamGameManager : MonoBehaviour, IGamePlayable
{
    ...
    [SerializeField] [Range(0, 3)] private float horizontalSpeed = 2.5f;

    [SerializeField] [Range(0, 3)] private float verticalSpeed = 2.5f;
    ...
    private IEnumerator MoveVertical()
    {
        ...
        while (sec <= verticalSpeed * downSec)
        {
            sec += Time.fixedDeltaTime;
            Vector3 switchPosition = originPos;
            switchPosition.y = Mathf.Lerp(originPos.y, destTransform.position.y, sec / (verticalSpeed * downSec));
        ...
        sec = 0;
        var upSec = 3f;
        while (sec <= verticalSpeed * upSec)
        {
            sec += Time.fixedDeltaTime;
            Vector3 switchPosition = originPos;
            switchPosition.y = Mathf.Lerp(destTransform.position.y, originPos.y, sec / (verticalSpeed * upSec));
            iceCream.position = switchPosition;
            yield return waitForFixedUpdate;
        }
