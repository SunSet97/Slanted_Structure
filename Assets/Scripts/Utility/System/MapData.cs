...
public class MapData : MonoBehaviour
{
    ...
    [Tooltip("CameraMoving 경계 설정")] public BoxCollider cameraBound; // 카메라

    [Header("BGM입니다")] public AudioClip BGM;
    ...
    void CreatePositionSetting(Character createWho)
    {
        ...
        Transform instant = new GameObject().transform;
        // 포지션 세팅 오브젝트 설정
        temp.posSet = Instantiate(instant, positionSetting.transform.position, Quaternion.identity,
            positionSetting.transform) as Transform;
        // 인덱스 설정
    ...
    {
        positionSets.Sort(delegate(CharacterPositionSet A, CharacterPositionSet B)
        {
