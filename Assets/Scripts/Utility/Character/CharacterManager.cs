...
#pragma warning restore 0649
        ...
        public void Init()
        {
            ...
            if (who.Equals(CharacterType.Speat) || who.Equals(CharacterType.Oun) || who.Equals(CharacterType.Rau))
            {
                faceExpression = Resources.LoadAll<Texture>("Face");
            }
            else if (who.Equals(CharacterType.Speat_Adolescene) || who.Equals(CharacterType.Speat_Adult) ||
                     who.Equals(CharacterType.Speat_Child))
            {
        ...
        public void SetCharacter(MapData.CharacterPositionSet posSet, CharacterData characterData = null)
        {
            ...
            if (DataController.Instance.CurrentMap.characterMoveType == CharacterMoveType.OneDirection &&
        ...
        public void MoveCharacter(CharacterMoveType characterMoveType, bool isJoystickInputUse)
        {
            ...
            if (!DataController.Instance.CurrentMap.isJoystickControlDisable && isJoystickInputUse)
            {
                ...
                else if (characterMoveType == CharacterMoveType.AllDirection)
                {
                    QuarterView(joystickDeltaAngle);
                }
                else if (characterMoveType == CharacterMoveType.Waypoint)
                {
                ...
                else if (characterMoveType == CharacterMoveType.Other)
                {
                    if (Mathf.Abs(joystickDeltaAngle) > 0)
                    {
                        transform.Rotate(Vector3.up, joystickDeltaAngle);
        ...
        {
            if (characterMoveType.Equals(CharacterMoveType.OneDirection))
            {
                CharacterAnimator.SetBool(TwoSideHash, true);
                // Debug.LogWarning("Set 2D true");
            }
            else if (characterMoveType.Equals(CharacterMoveType.AllDirection))
            {
                CharacterAnimator.SetBool(TwoSideHash, false);
                // Debug.LogWarning("Set 2D false");
            }
        ...
        {
            if (Application.isPlaying && DataController.Instance.GetCharacter(CharacterType.Main) == this)
            {
                var scale = 1f;
