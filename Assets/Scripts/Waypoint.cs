...
public class Waypoint : MonoBehaviour
{
    ...
    public int currentIndex; // 현재 체크 포인트 인덱스
    ...
    private void UpdateWaypoints()
    {
        if (waypoints.Count != transform.childCount)
        {
            ...
            waypoints.Sort(delegate (Transform A, Transform B) { if (String.Compare(A.name, B.name, StringComparison.Ordinal) > 0) return 1; else return -1; }); // 이름순으로 정렬
    ...
    private void WaypointCheck()
    {
        ...
        float minDist = DistanceIgnoreY(character.position, waypoints[currentIndex].position); // 현재 체크 포인트의 거리를 계산후 최단거리 설정
        ...
        if (backDis <= minDist) {nearestPoint = waypoints[currentIndex - 1]; minDist = backDis;}
        if (frontDis <= minDist) {nearestPoint = waypoints[currentIndex + 1]; minDist = frontDis;}


        if (DistanceIgnoreY(character.position, nearestPoint.position) <= checkingDist) checkedWaypoint = nearestPoint; // 캐릭터와 waypoint의 거리가 check 가능한 거리가 되면 해당 포인트를 check해줌
    }
    ...
    private void JoystickInput()
    {
        ...
        if (DataController.instance_DataController.joyStick.gameObject.activeSelf && waypoints.Count > 0 &&
            DataController.instance_DataController.GetCharacter(DataController.CharacterType.Main).IsMove)
        {
            ...
            if (movingDir == moveDirSet)
            {
                if (isIn)
                {
                    moveDir = (waypoints[fwdIndex].position - waypoints[currentIndex].position).normalized * checkingDist -
            ...
            else
            {
                if (isIn)
                {
                    moveDir = (waypoints[bwdIndex].position - waypoints[currentIndex].position).normalized * checkingDist -
                              (waypoints[currentIndex].position - character.position);
                }
                else
                {
